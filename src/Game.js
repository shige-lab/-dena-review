...
const Square = (props) => {
	...
	const Board = ()=> {
		...
		const handle_turn = (col,row) => {
			...
			if (square[col][row] == null && can_put(square,col, row) == true && status != 'Winner: ')
			{
				square[col][row] = isnext ? 'X' : 'O';
				setSquares(square);
				if (Board_Scan(col, row, square) == false)
					setIsnext(!isnext);
				// CheckWin(square);
			};
		...
		const Board_Scan_Sub = (  x,  y,  move_x,  move_y, square ) => {
			...
			for( i = 1; i < 5; i++ ){
				if (x + (move_x * i) > 5 || y + (move_y * i) > 6 || x + (move_x * i) < 0 || y + (move_y * i) < 0) break;
				if( square[ x + (move_x * i) ][ y + (move_y * i) ] == square[x][y] ) n += 1;
				else break;
			}
			for( i = 1; i < 5; i++ ){
				if (x + (-1*move_x * i) < 0 || y + (-1*move_y * i) < 0 || x + (-1*move_x * i) > 5 || y + (-1*move_y * i) > 6) break;
				if( square[ x + (-1*move_x * i) ][ y + (-1*move_y * i) ] == square[x][y] ) n += 1;
				else break;
			}
		...
		// 		for (let i = 0 ; i < 6 ; i++)
		// 		{
		// 			if (square[k][i] == square[k][i + 1] && square[k][i] != null)
		// 				row_flag += 1;
		// 			else
		// 				row_flag = 0;
		// 			if (row_flag == 3)
		...
		// 		for (let k = 0 ; k < 5 ; k++)
		// 		{
		// 			if (square[k][i] == square[k + 1][i] && square[k][i] != null)
		// 				col_flag += 1;
		// 			else
		// 				col_flag = 0;
		// 			if (col_flag == 3)
		...
		// }
			...
			return (
			<div>
		  <div className="status">{status}{isnext ? 'X' : 'O'}</div>
...
//   ReactDOM.render(
// 	<Game />,
// 	document.getElementById('game_test')
//   );
